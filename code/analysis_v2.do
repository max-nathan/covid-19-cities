...
- Add in Google  mobility info + Google / ONS crosswalk for mobility data 
- something on mobility response wrt occupational breakdowns by area 
...
corr $lhs allages sh_under_30 sh_60_plus sh_31_59 sh_male 
corr $lhs people_per_hh peoplepersqkm_2018 sh_overcrowded_hhs sh_public_transport 
corr $lhs median_wage_all_workers sh_grads sh_selfemployed sh_unemployed sh_inactive sh_senior sh_pros sh_assoc_pro_tech sh_admin sh_trades sh_clos sh_scs sh_ppm 
...
est store a		
qui reg log_$cases log_peoplepersqkm_2018 $urban $imd $crdum, cl($cl) 	
est store a_u	
qui reg log_$cases log_peoplepersqkm_2018 $dem  $crdum, cl($cl) 	
est store a_d
qui reg log_$cases log_peoplepersqkm_2018 $econ  $crdum, cl($cl) 	
est store a_e
qui reg log_$cases log_peoplepersqkm_2018 $urban $dem $econ $imd $crdum, cl($cl) 	
est store a_all	
	
coefplot a_u, sort(,d) drop(_cons) 	
coefplot a_d, sort(,d) drop(_cons) 
coefplot a_all, sort(,d) drop(_cons)
