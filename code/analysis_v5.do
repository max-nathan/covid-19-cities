...
global home 		$drop/02_Submitted/P20_COVID-19_cities/covid-19-cities 	// edit path as needed 
...
global ldate 		22209 	// 21 October 
global lockdown		21997	// 23 March
global easing 		22100	// 4 July  			
...
global imd 			IMDRankofaveragescore	
...
set scheme cleanplots 		// for colour reproductions 
set scheme lean2			// for b+w reproductions 
...
scatter $lhs sh_70_plus if date==$ldate [aw = peopleper], mc(blue%80)						///
	...
	ylab(5(0.5)8.5, labs(small) angle(horizontal)) ylab(,nogrid)  							///	
	...
	note("Source: PHE, ONS. Confirmed hospital cases as of 21 October. Weighted by population density.", size(vsmall))
	...
	* raw cases
	...
	foreach var in cumulativelabconfirmedcases {	
		xtline `var' if date<=$ldate, overlay i(cr_id) t(date) graphr(c(white)) 				///
			...
			title("Total cases, English UTLAs", size(small))	xline(21997 22100)				///
			note("Source: PHE. Confirmed hospital cases as of 21 October.", size(vsmall))
	...
	foreach var in x  {	
		xtline `var' if date<=$ldate, overlay i(cr_id) t(date) graphr(c(white)) 				///
			...
			legend(	label(1 "Birmingham") label (2 "Bristol") label(3 "Leeds") 					///
				...
				label(7 "Newcastle-Gateshead") label (8 "Sheffield") label (9 "Tees Valley")	///
				label(10 "Rest of England total") cols(1) size(vsmall))							///
			title("Cases per 100k people, English UTLAs", size(small)) xline(21997 22100)		///				
			note("Source: PHE. Confirmed hospital cases as of 21 October.", size(vsmall))	
		graph export "$results/cr_`var'.png", as(png) replace 
		}
...
graph two scatter cumulative_cases_100k date  if date<=$ldate, 					///
	...
	by(city_region, r(2) 														///
	note("Source: Public Health England data for England, 30 January - 21 October. Confirmed hospital cases per 100,000 people for local authorities within city-regions." "City-regions defined using combined authority boundaries and GLA boundary for London.", size(vsmall)))	 				
...
graph combine 	binscatter_$lockdown.gph  binscatter_$ldate.gph,					///	
				...
				note("Source: PHE, ONS. Confirmed hospital cases as of 23 March (L) vs 21 October (R).", size(vsmall))		
...
graph combine 	binscatter_c_$lockdown.gph  binscatter_c_$ldate.gph,				///				
				title("Cases per 100,000 people | population density", size(small))	///	
				note("Source: PHE, ONS, Census, ASHE, APS. Confirmed hospital cases as of 23 March (L) vs 21 October (R)." "Urban, demographic, IMD, occupation and labour market controls included", size(vsmall))		
...
graph combine 	scatter_imd_$lockdown.gph  scatter_imd_$ldate.gph,					///				
				...
				note("Source: PHE, MHCLG.  Confirmed hospital cases as of 23 March (L) vs 21 October (R). IMD Rank of average score, where 1 is most deprived.", size(vsmall))		
