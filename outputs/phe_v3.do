...
PHE data now here, figure out a way to automate download: https://coronavirus.data.gov.uk/#local-authorities
...
- date variable in stata for graphs, redo 
- complete scatters 
- try some regressionds, redo binscatters 
 
- PHE death data 
- controls: % essential occs [ask mike b], % workplace flex	[aps], % BAME, % hhs living with over 70s, travel time, airports
- Google + Apple mobility info 
- Google / ONS crosswalk for mobility data 

- split out syntax => repo 
- master: build, analysis 
- build: phe, controls, mobility, make panel  
- analysis: timeseries, scatters / c-matrix, regressions and binscatters  
...
global phe_cases 		$data/phe/coronavirus-cases_$logdate	
global phe_deaths 		$data/phe/coronavirus-deaths_$logdate
...
save "$data/phe/phe_cases_$logdate.dta", replace

u "$data/phe/phe_cases_$logdate.dta", clear 

keep if areatype=="Upper tier local authority"
ren areacode lad_code
ren areaname lad_name

* make a stata date variable here from specimendate
* split out date fields 
...
so lad_code month day year 
egen id = group(lad_code)
order id lad* year month day dailylabconfirmedcases cumulativelabconfirmedcases
...
compress
save $data/tidy/phe.dta, replace
...
label var v5 "median wage part-time workers 2019"
...
gen c = median_wage_all_workers if lad_code=="E06000052" 			// merge cornwall and isles of scilly 
gen ios = median_wage_all_workers if lad_code=="E06000053"
replace median_wage_all_workers = c+ios if lad_code=="E06000052" 
drop if lad_code=="E06000053"
gen col = median_wage_all_workers if lad_code =="E09000001"			// merge city of london and hackney 
gen h = median_wage_all_workers if lad_code =="E09000012"
replace median_wage_all_workers = col+h if lad_code=="E09000012" 
drop if lad_code=="E09000001"	
drop c ios col h

gen c = median_wage_ft_workers if lad_code=="E06000052" 			// merge cornwall and isles of scilly 
gen ios = median_wage_ft_workers if lad_code=="E06000053"
replace median_wage_ft_workers = c+ios if lad_code=="E06000052" 
drop if lad_code=="E06000053"
gen col = median_wage_ft_workers if lad_code =="E09000001"			// merge city of london and hackney 
gen h = median_wage_ft_workers if lad_code =="E09000012"
replace median_wage_ft_workers = col+h if lad_code=="E09000012" 
drop if lad_code=="E09000001"	
drop c ios col h

gen c = median_wage_pt_workers if lad_code=="E06000052" 			// merge cornwall and isles of scilly 
gen ios = median_wage_pt_workers if lad_code=="E06000053"
replace median_wage_pt_workers = c+ios if lad_code=="E06000052" 
drop if lad_code=="E06000053"
gen col = median_wage_pt_workers if lad_code =="E09000001"			// merge city of london and hackney 
gen h = median_wage_pt_workers if lad_code =="E09000012"
replace median_wage_pt_workers = col+h if lad_code=="E09000012" 
drop if lad_code=="E09000001"	
drop c ios col h
...
gen cases_pop = daily/Allages
gen dailycases_100k = cases_pop*1000
gen dailycases_1k = cases_pop*100000
drop cases_pop 
label var dailycases_100k "Daily confirmed cases per 100k people"
label var dailycases_1k "Daily confirmed cases per 1000 people"
gen cases_pop = cumulative/Allages
gen cumu_cases_100k = cases_pop*1000
gen cumu_cases_1k = cases_pop*100000
drop cases_pop 
label var cumu_cases_100k "Cumulative confirmed cases per 100k people"
label var cumu_cases_1k "Cumulative confirmed cases per 1000 people"

ren Allages allages 
order id lad_code lad_name month daily* cumu* allages male age* sh*  
...
compress
drop _m areaname 
order id lad_code lad_name city_region specimen year 
...
* !!!!! make a date variable in stata, redo this 
* compare city-region average with rest of England average 



*--- 1/ Graph timeseries for all LAs ---//
...
*---- 2/ Cases vs area characteristics ---//
...
* cumulative cases as of 18 april 

corr cumulative allages sh_under_30 sh_60_plus sh_31_59 sh_male if specimen=="18/04/2020"
corr cumulative people_per_hh peoplepersqkm_2018 sh_overcrowded_hhs sh_public_transport if specimen=="18/04/2020"
corr cumulative median_wage_all_workers sh_grads sh_selfemployed sh_unemployed sh_inactive sh_senior sh_pros sh_assoc_pro_tech sh_admin sh_trades sh_clos sh_scs sh_ppm if specimen=="18/04/2020"
...
*--- Scatter #cases against area characteristics ---// [to do!!!!!!!!!! demography / density / occupations]
