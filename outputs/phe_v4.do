...
* deprecated 
* now downloaded directly from phe website, find a way to automate this [https://coronavirus.data.gov.uk/#local-authorities]
...
gen cumulcases_100k = cases_pop*1000
gen cumulcases_1k = cases_pop*100000
drop cases_pop 
label var cumulcases_100k "Cumulative confirmed cases per 100k people"
label var cumulcases_1k "Cumulative confirmed cases per 1000 people"
...
so id date cumulative 
xtline cumulative , overlay i(lad_name) t(date) 						///
	legend(off) graphr(c(white)) ylab(,nogrid) 							///
	note("Source: PHE. Data extracted $logdate", size(vsmall))

xtline cumulative if cumulative>5, overlay i(lad_name) t(date) 
