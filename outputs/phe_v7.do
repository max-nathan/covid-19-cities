...
- fgraph schemes ~ try cleanplots?
- ffinalise scatters: 
	scatter all 3 density vars on same day; show change over time 
	scatter 3 of dem, occ, wages 
- finalise binscatters: show change over time for start, middle, end  
...
* makes ... 
* city-region level: total cases: London ~ rest of England outside major cities 
* city-region level: average cases: average city-region LA vs average other LA 
* change within a city-region 
...
preserve 
	...
	foreach var in cumulative_cases_100k {
		...
		graph export "$results/cr_`var'_$logdate.png", as(png) replace 
...
preserve 
	...
	foreach var in cumulative_cases_100k {
		...
		graph export "$results/cr_ave_`var'_$logdate.png", as(png) replace 
...
graph two scatter cumulative_cases_100k date, 											///
	by(city_region, r(2) note("Source: PHE. Data extracted $logdate", size(vsmall))) 	///
...
graph two scatter cumulative_cases_100k date if city_region!="rest of England", 		///
	by(city_region, r(3) note("Source: PHE. Data extracted $logdate", size(vsmall))) 	///
	ytitle("Cumulative cases / 100k", size(small)) ylab(,nogrid) 						///
	xtitle(, size(small)) xlab(none)  xtitle("")										///	
	msymbol(circle_hollow) msize(tiny) 
...
*--- 3/ Highlighting change in / within a given LA/city-region, e.g. Birmingham ---//

preserve

	drop id 
	egen id = group(lad_code)
	keep cumulative_cases_100k date id lad_name
	qui reshape wide cumulative_cases_100k lad_name, i(date) j(id) 	

	forval id= 1/149 {
	local lp  `lp' line cumulative_cases_100k`id' date, lc(gs15%50) ||
		}
	twoway `lp' || 													///
		line cumulative_cases_100k56 date, lc(orange) ||			///
		line cumulative_cases_100k57 date, lc(orange) ||			///
		line cumulative_cases_100k58 date, lc(orange) ||			///
		line cumulative_cases_100k59 date, lc(orange) ||			///	
		line cumulative_cases_100k60 date, lc(orange) ||			///
		line cumulative_cases_100k61 date, lc(orange) ||			///
		line cumulative_cases_100k62 date, lc(orange) ||			///
		line cumulative_cases_100k63 date, lc(orange) ||			///
		line cumulative_cases_100k64 date, lc(orange) ||			///
		line cumulative_cases_100k65 date, lc(orange) 				///
	 ...
	 graph export "$results/gmlads_`var'_$logdate.png", as(png) replace 
...
so id date cumulative_cases_100k 
line cumulative_cases_100k date if city_region=="Manchester" & cumulativelabconfirmedcases>=5, lc(orange) || 			///
	line cumulative_cases_100k date if city_region!="Manchester" & cumulativelabconfirmedcases>=5, lc(gs15%65)			///
	graphr(c(white)) 																	///
	ytitle(, size(small)) ylab(, labs(small) angle(horizontal)) ylab(,nogrid)  			///	
	xtitle(, size(small)) xlab(, labs(small) angle(45)) 								///
	legend(label(1 "Manchester city-region") label (2 "Rest of England") size(small)) 	///
	note("Source: PHE. Data extracted $logdate", size(vsmall))
...
so id date cumulative_cases_100k 
line 	cumulative_cases_100k date if lad_name=="Bolton", lc(orange)	||				///
		line cumulative_cases_100k date if lad_name=="Bury", lc(orange)	||				///
		line cumulative_cases_100k date if lad_name=="Manchester", lc(orange) 	|| 		///
		line cumulative_cases_100k date if lad_name=="Oldham", lc(orange)	||			///
		line cumulative_cases_100k date if lad_name=="Rochdale", lc(orange)	||			///
		line cumulative_cases_100k date if lad_name=="Salford", lc(orange)	||			///
		line cumulative_cases_100k date if lad_name=="Stockport", lc(orange) ||			///
		line cumulative_cases_100k date if lad_name=="Tameside", lc(orange)	||			///
		line cumulative_cases_100k date if lad_name=="Trafford", lc(orange)	||			///
		line cumulative_cases_100k date if lad_name=="Wigan", lc(orange)	||			///
		line cumulative_cases_100k date if city_region!="Manchester", lc(gs15%65)		///
		graphr(c(white)) 																///
		ytitle(, size(small)) ylab(, labs(small) angle(horizontal)) ylab(,nogrid)  		///	
		xtitle(, size(small)) xlab(, labs(small) angle(45)) 							///
		legend(off)																		///
...
so id date
line cumulative_cases_100k date if lad_name=="Birmingham", lc(orange) || 						///
	line cumulative_cases_100k date if lad_name!="Birmingham" & cumulativelab>5, lc(gs15%65)		///
	graphr(c(white)) 																	///
	ytitle(, size(small)) ylab(, labs(small) angle(horizontal)) ylab(,nogrid)  			///	
	xtitle(, size(small)) xlab(, labs(small) angle(45)) 								///
...
foreach var in cumulative_cases_100k peoplepersqkm_2018 median_wage_all_workers people_per_hh sh_overcrowded_hhs sh_public_transport  {
...
foreach var in log_peoplepersqkm_2018 sh_overcrowded_hhs sh_public_transport { 

	scatter log_cumulative_cases_100k `var' if date==22026 				///
			|| lfit log_cumulative_cases_100k `var' if date==22026, 	///
			ytitle("log cumulative cases / 100k", size(small)) 			///
			ylab(, labs(small) angle(horizontal)) ylab(,nogrid)  		///	
			xtitle(, size(small)) xlab(, labs(small) angle(45)) 		///		
			legend(off)	graphr(c(white)) 								///
			note("Source: PHE. Date = !!!!! Data extracted $logdate", size(vsmall))			

		}	
	
/*	
* urban / density over time 

foreach date in 21986 21997 22026 { 

	scatter log_cumulative_cases_100k log_peoplepersqkm_2018 if date==`date' 				///
			|| lfit log_cumulative_cases_100k log_peoplepersqkm_2018 if date==`date', 	///
			ytitle("log cumulative cases / 100k", size(small)) 			///
			ylab(, labs(small) angle(horizontal)) ylab(,nogrid)  		///	
			xtitle(, size(small)) xlab(, labs(small) angle(45)) 		///		
			legend(off)	graphr(c(white)) 								///
...
foreach var in sh_60_plus sh_male { 
	...
	scatter cumulative_cases_100k `var' if date==22026 						///
			|| lfit cumulative_cases_100k `var' if date==22026, 			///
			graphr(c(white)) 										///
...
foreach var in sh_clos sh_scs sh_ppm  { 
...
reg log_cumulative_cases_100k log_peoplepersqkm_2018 if date==21997, cl($cl) 	// 23 march - lockdown date
...
foreach date in 21986 21997 22026 {	
	...
	binscatter log_cumulative_cases_100k log_peoplepersqkm_2018 if date==`date', 	///
		...
		ytitle(, size(small)) xtitle(, size(small))					///
		ylab(0(1)6, labs(small) angle(horizontal)) ylab(,nogrid)  	///	
		xlab(5(1)10, labs(small) angle(horizontal)) 						///	
		note("Source: PHE. Data extracted $logdate. Date = `date'", size(vsmall))	
	}
